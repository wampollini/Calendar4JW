...
<p><strong>� Scansione QR Nextcloud:</strong> Puoi anche scansionare il codice QR generato da Nextcloud (Impostazioni → Sicurezza → Dispositivi e sessioni → Genera password applicazione) per configurare automaticamente la connessione CalDAV.</p>
...
<p><strong>� Escaneo QR Nextcloud:</strong> También puedes escanear el código QR generado por Nextcloud (Configuración → Seguridad → Dispositivos y sesiones → Generar contraseña de aplicación) para configurar automáticamente la conexión CalDAV.</p>
...
<p><strong>� Nextcloud QR Scan:</strong> You can also scan the QR code generated by Nextcloud (Settings → Security → Devices and sessions → Generate app password) to automatically configure the CalDAV connection.</p>
